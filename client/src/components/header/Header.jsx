  ...
  return (
    <header className="w-full">
      <div className="flex px-4 py-3 text-white justify-between  bg-blue-900 items-center w-full">
        <div className="font-bold text-sm sm:text-md md:text-lg nowrap flex items-center gap-x-3">
          <Link to="/">Shop X</Link>
          <button onClick={toggleSidebar} className="flex text-lg lg:hidden">
            <GiHamburgerMenu />
          </button>
        </div>
        ...
        {isLogged && userData ? (
          ...
          <div className="px-0 sm:px-2 flex items-center justify-between gap-x-2 sm:w-[20%] md:w-[17%] lg:w-[12%]">
            {!location.pathname.includes("/login") && (
              <Link
                to="/login"
                className="text-sm font-bold sm:text-md border rounded-md px-2 py-1 md:px-2 md:py-1 text-blue-900 bg-white hover:text-white hover:bg-blue-800"
              >
                Login
              </Link>
            )}
